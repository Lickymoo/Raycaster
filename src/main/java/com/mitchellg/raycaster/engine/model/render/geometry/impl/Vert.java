...
public class Vert extends Geometry {
    ...
    public void generateBoundingBox(Vector3f offset, Vector3f rotation, Vector3f scale){
        Vector3f vertex0 = v0.rotate(rotation).multiply(scale).add(offset);
        Vector3f vertex1 = v1.rotate(rotation).multiply(scale).add(offset);
        Vector3f vertex2 = v2.rotate(rotation).multiply(scale).add(offset);
    }
